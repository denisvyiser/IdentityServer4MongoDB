...
namespace System.Linq
{
    public static class QueryableExtensions
    {
            ...
            where TEntity : class
        {
            ...
            try
            {
                var filterExpression = Expression.Constant(restriction.Value);
                ...
                switch (restriction.Condition)
                {
                    ...
                    case Condition.Default:
                        expression = Expression.Call(property, property.Type.GetMethod("ToString", Type.EmptyTypes));                       
                        MethodInfo method = typeof(string).GetMethod("Contains", new[] { typeof(string) });                        
                        expression = Expression.Call(expression, method, filterExpression);
                        break;
                }
